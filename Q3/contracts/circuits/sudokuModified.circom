...
template sudoku() {
    ...
    // array of components
    component RangeCheckers[162];
    for (var i = 0 ; i< 162 ; i++ )
    {
        RangeCheckers[i] = RangeProof();
    }

    // long array, puzzle from 0 to 80, solution from 81 to 161
    
    // looping variables to access array of components to validate ranges
    var k = 0;
    var v = 81;
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            RangeCheckers[k].range[0] <== 0;
            //[assignment] change assert() to use your created RangeProof instead
            RangeCheckers[k].range[1] <== 9;

            RangeCheckers[k].in <== puzzle[i][j];
            assert (RangeCheckers[k].out == 1);

            k = k+1 ;
            
            //[assignment] change assert() to use your created RangeProof instead
            RangeCheckers[v].range[0] <== 0;

            //[assignment] change assert() to use your created RangeProof instead
            RangeCheckers[v].range[1] <== 9;

            RangeCheckers[v].in <== solution[i][j];
            assert (RangeCheckers[v].out == 1);
